...
bool gli::m_3D=true; //render 3d scene, default
float gli::m_2d_view[4]={0,0,0,0};
...
glm::mat4 gli::getViewMatrix()
{
	glm::mat4 M;
	M = glm::rotate(M, m_currentAzim + m_deltaAzim, glm::vec3(0.0, 1.0, 0.0));
	M = glm::rotate(M, m_currentElev + m_deltaElev, glm::vec3(1.0, 0.0, 0.0));

	glm::vec3 trans = glm::vec3((m_CameraPos[0] + m_deltaDis[0]), -(m_CameraPos[1] + m_deltaDis[1]), -(m_CameraPos[2] + m_deltaDis[2]));
...
void gli::gliDisplay( void )
{
	...
	GLint view_w = viewport[2];
	GLint view_h = viewport[3];

	//Draw scene
	if(m_3D){
		glMatrixMode( GL_PROJECTION );
    	glLoadIdentity();
		gluPerspective(60, view_w*1.0 / view_h, 0.1, 1500);
		...
		glLoadIdentity();
		glOrtho(m_2d_view[0],m_2d_view[1],m_2d_view[2],m_2d_view[3],-1000,1000);
