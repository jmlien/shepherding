...
public:
    //Constructor
    CObs(const std::string& filename, int size);
    ~CObs();

    ///////////////////////////////////////////////////////////////////////////
    bool buildGLModel()
    { 
        if( m_Model==NULL ) return true;  //has been build
        if( m_Obs.buildGLModel(*m_Model)==false ) throw;
